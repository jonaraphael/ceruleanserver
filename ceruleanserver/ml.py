...
def machine(learner, snso):
    ...
    img_to_chips(img_path, inf_dir, chip_size_orig, chip_size_reduced, img_path.stem, overhang, max_chip_qty, start_over, record_nonzeros, learner, threshold)
...
def crop_box_gen(px_wide, px_high, chip_size, geo_transform=(0,1,0,0,0,1), overhang=False):
    ...
    for i in range(num_wide):
        for j in range(num_high):
            x_vals = [i*chip_size, (i+1)*chip_size] if (i!=num_wide-1) or overhang else [px_wide-chip_size, px_wide]
            y_vals = [j*chip_size, (j+1)*chip_size] if (j!=num_high-1) or overhang else [px_high-chip_size, px_high]
            yield [
                geo_transform[0]+x_vals[0]*geo_transform[1]+y_vals[0]*geo_transform[2], # ULX: X_origin + box_number * num_pixels_per_box * size_of_X_pixels + 0
                geo_transform[3]+x_vals[0]*geo_transform[4]+y_vals[0]*geo_transform[5], # ULY: Y_origin + 0 + box_number * num_pixels_per_box * size_of_Y_pixels
                geo_transform[0]+x_vals[1]*geo_transform[1]+y_vals[1]*geo_transform[2], # LRX
                geo_transform[3]+x_vals[1]*geo_transform[4]+y_vals[1]*geo_transform[5]] # LRY

def img_to_chips(img_path, out_dir, chip_size_orig, chip_size_reduced, out_stem=None, overhang=False, max_chip_qty=None, start_over=True, record_nonzeros=False, mllearner=None, threshold=None):
    ...
    if not img_path.exists(): # pylint: disable=no-member
        print('ERROR: No image found:', img_path) # We need an image if you want to chip it
        return
    if config.VERBOSE: print("Chipping", img_path.name, "into", out_dir)
    out_dir.mkdir(parents=True, exist_ok=True) # Create new directory for chips to be stored
    out_stem = out_stem or img_path.stem # Default the output names to be the same as the input image

    img = gdal.Open(str(img_path)) # Open the large image as a GDAL DataSet
    boxes = crop_box_gen(img.RasterXSize, img.RasterYSize, chip_size_orig, img.GetGeoTransform(), overhang) # A list of pairs of corners (UL & LR) which will be cut from the large GeoTiff
    s = datetime.now() # Measure processing time
    for i, b in enumerate(boxes): # for each box
        if (max_chip_qty is None) or (i<max_chip_qty): # Stop short if artificially avoiding too many chips
            out_path = out_dir/f'{out_stem}_{i}.png' # Create indexed name for new png to be stored
            if start_over or not out_path.exists(): # Avoid overwriting images already made
                gdal.Translate(str(out_path), img, format="PNG", width=chip_size_reduced, height=chip_size_reduced, projWin=b, outputType=gdal.GDT_Byte) # check out more args here: https://gdal.org/python/osgeo.gdal-module.html#TranslateOptions
                if mllearner: # If model provided
                    infer(out_path, mllearner, threshold) # Run Inference on the current chip
                if record_nonzeros: # If recording nonzero chips
                    chp = gdal.Open(str(out_path)) # Open each chip
                    stats = chp.GetRasterBand(1).GetStatistics(0,1) # Calculate the stats on that chip
        ...
        if config.VERBOSE: print(out_path.stem, (datetime.now()-s)/(i+1)) # Inference around 6.7s per 512px chip
    del img
