...
class SHO:
    def __init__(self, sns_msg, user=config.SH_USER, pwd=config.SH_PWD):
        ...
        self.URLs = {"query_prods" : f"https://{self.user}:{self.pwd}@scihub.copernicus.eu/apihub/search?rows=100&q=(platformname:Sentinel-1 AND filename:{self.generic_id})",}
        ...
        if self.ocn:
            self.URLs["download_ocn"] = f"https://{self.user}:{self.pwd}@scihub.copernicus.eu/dhus/odata/v1/Products('{self.ocn_shid}')/%24value"
    ...
    def download_grd_tiff(self): 
        if not (self.grd and self.s3["grd_tiff"]): 
    ...
    def update_intersection(self, ocean_shape):
        ...
        self.ready = self.isoceanic and self.isvv
